...
def extract_features(folder):
    for family in glob.iglob(f'{folder}/*'):
        for members in glob.iglob(f'{family}/*'):
            ...
            # print(len(num_files))
            members_img_array = np.zeros((len(num_files), resnet_output))
            for i, photos in enumerate(glob.iglob(f'{members}/*.jpg')):
                ...
                model_pt.eval()
                output = model_pt(img)
                members_img_array[i] = output[0].detach().numpy()
            ...
            member_mean = np.pad(member_mean, (0, 50176 - len(member_mean)), 'constant')
            member_mean = member_mean.reshape(width, height)
